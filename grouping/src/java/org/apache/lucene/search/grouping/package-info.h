 ...
 *
 * <p>Groups are defined by {@link
 * org.apache.lucene.search.grouping.GroupSelector} implementations:</p> <ul>
 *     <li>{@link org.apache.lucene.search.grouping.TermGroupSelector} groups
 * based on the value of a {@link org.apache.lucene.index.SortedDocValues}
 * field</li> <li>{@link
 * org.apache.lucene.search.grouping.ValueSourceGroupSelector} groups based on
 *     the value of a {@link
 * org.apache.lucene.queries.function.ValueSource}</li>
 *   </ul>
 *
 ...
 * </ul>
 *
 * <p>Typical usage for the generic two-pass grouping search looks like this
 * using the grouping convenience utility (optionally using caching for the
 * second pass search):</p>
 *
 ...
 *   TopGroups&lt;BytesRef&gt; result = groupingSearch.search(indexSearcher,
 ...
 *
 * <pre class="prettyprint">
 *   // Set this once in your app &amp; save away for reusing across all
 * queries: Filter groupEndDocs = new CachingWrapperFilter(new
 ...
 * groupOffset+topNGroups, needsScores, groupEndDocs); s.search(new
 * TermQuery(new Term("content", searchTerm)), c); TopGroups groupsResult =
 * c.getTopGroups(withinGroupSort, groupOffset, docOffset,
 ...
 *
 * <p>Another collector is the <code>AllGroupHeadsCollector</code> that can be
 * used to retrieve all most relevant documents per group. Also known as group
 * heads. This can be useful in situations when one wants to compute group based
 * facets / statistics on the complete query result. The collector can be
 * executed during the first or second phase. This collector can also be used
 * with the <code>GroupingSearch</code> convenience utility, but when if one
 * only wants to compute the most relevant documents per group it is better to
 * just use the collector as done here below.</p>
 *
 * <pre class="prettyprint">
 *   TermGroupSelector grouper = new TermGroupSelector(groupField);
 *   AllGroupHeadsCollector c = AllGroupHeadsCollector.newCollector(grouper,
 * sortWithinGroup); s.search(new TermQuery(new Term("content", searchTerm)),
 * c);

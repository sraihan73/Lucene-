 ...
 */
class LuceneVarGapDocFreqInterval final : public PostingsFormat
{
  GET_CLASS_NAME(LuceneVarGapDocFreqInterval)
public:
  const int termIndexInterval;
  const int docFreqThreshold;

  LuceneVarGapDocFreqInterval();

  LuceneVarGapDocFreqInterval(int docFreqThreshold, int termIndexInterval);
...
protected:
  std::shared_ptr<LuceneVarGapDocFreqInterval> shared_from_this()
  {
    return std::static_pointer_cast<LuceneVarGapDocFreqInterval>(

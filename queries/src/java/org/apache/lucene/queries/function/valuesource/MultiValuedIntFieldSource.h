 ...
 */
class MultiValuedIntFieldSource : public IntFieldSource
{
  GET_CLASS_NAME(MultiValuedIntFieldSource)

protected:
  const SortedNumericSelector::Type selector;

public:
  MultiValuedIntFieldSource(const std::wstring &field,
...
protected:
  std::shared_ptr<MultiValuedIntFieldSource> shared_from_this()
  {
    return std::static_pointer_cast<MultiValuedIntFieldSource>(
        IntFieldSource::shared_from_this());
  }
};

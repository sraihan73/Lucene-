 ...
 */
class MultiValuedLongFieldSource : public LongFieldSource
{
  GET_CLASS_NAME(MultiValuedLongFieldSource)

protected:
  const SortedNumericSelector::Type selector;

public:
  MultiValuedLongFieldSource(const std::wstring &field,
...
protected:
  std::shared_ptr<MultiValuedLongFieldSource> shared_from_this()
  {
    return std::static_pointer_cast<MultiValuedLongFieldSource>(
        LongFieldSource::shared_from_this());
  }
};

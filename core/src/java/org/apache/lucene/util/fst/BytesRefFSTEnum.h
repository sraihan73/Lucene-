...
template <typename T>
class BytesRefFSTEnum final : public FSTEnum<T>
{
  GET_CLASS_NAME(BytesRefFSTEnum)
private:
  // C++ NOTE: Fields cannot have the same name as methods:
  const std::shared_ptr<BytesRef> current_ = std::make_shared<BytesRef>(10);
  const std::shared_ptr<InputOutput<T>> result =
      std::make_shared<InputOutput<T>>();
  std::shared_ptr<BytesRef> target;
...
public:
   ...
   *  term before target.  */
public:
  BytesRefFSTEnum(std::shared_ptr<FST<T>> fst) : FSTEnum<T>(fst)
  {
  ...
  seekCeil(std::shared_ptr<BytesRef> target) 
  {
    this->target = target;
  ...
  std::shared_ptr<InputOutput<T>>
  seekFloor(std::shared_ptr<BytesRef> target) 
  {
    this->target = target;
    targetLength = target->length;
  ...
  std::shared_ptr<InputOutput<T>>
  seekExact(std::shared_ptr<BytesRef> target) 
  {
    this->target = target;
    targetLength = target->length;
...
protected:
  ...
  int getCurrentLabel() override
  {
    ...
    return current_->bytes[upto] & 0xFF;
  }

  void setCurrentLabel(int label) override
  {
    current_->bytes[upto] = static_cast<char>(label);
  }

  void grow() override
  {
    current_->bytes = ArrayUtil::grow(current_->bytes, upto + 1);
  }
...
protected:
  std::shared_ptr<BytesRefFSTEnum> shared_from_this()
  {
    return std::static_pointer_cast<BytesRefFSTEnum>(

...
class ReverseBytesReader final : public FST::BytesReader
{
  GET_CLASS_NAME(ReverseBytesReader)
private:
  std::deque<char> const bytes;
  int pos = 0;

public:
  ReverseBytesReader(std::deque<char> &bytes);
...
protected:
  std::shared_ptr<ReverseBytesReader> shared_from_this()
  {
    return std::static_pointer_cast<ReverseBytesReader>(

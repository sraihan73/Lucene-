 ...
 * @lucene.internal
 */
class Direct32 final : public PackedInts::MutableImpl
{
  GET_CLASS_NAME(Direct32)
public:
  std::deque<int> const values;

  Direct32(int valueCount);

  Direct32(int packedIntsVersion, std::shared_ptr<DataInput> in_,
           int valueCount) ;
...
protected:
  std::shared_ptr<Direct32> shared_from_this()
  {
    return std::static_pointer_cast<Direct32>(

 ...
 * @lucene.internal
 */
class Direct8 final : public PackedInts::MutableImpl
{
  GET_CLASS_NAME(Direct8)
public:
  std::deque<char> const values;

  Direct8(int valueCount);

  Direct8(int packedIntsVersion, std::shared_ptr<DataInput> in_,
          int valueCount) ;
...
protected:
  std::shared_ptr<Direct8> shared_from_this()
  {
    return std::static_pointer_cast<Direct8>(

 ...
 * @lucene.internal
 */
class Direct16 final : public PackedInts::MutableImpl
{
  GET_CLASS_NAME(Direct16)
public:
  std::deque<short> const values;

  Direct16(int valueCount);

  Direct16(int packedIntsVersion, std::shared_ptr<DataInput> in_,
           int valueCount) ;
...
protected:
  std::shared_ptr<Direct16> shared_from_this()
  {
    return std::static_pointer_cast<Direct16>(

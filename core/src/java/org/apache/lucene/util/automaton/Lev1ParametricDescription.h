...
class Lev1ParametricDescription : public ParametricDescription
{
  GET_CLASS_NAME(Lev1ParametricDescription)
...
private:
  ...
  // 4 vectors; 5 states per deque; array length = 20
  static std::deque<int64_t> const toStates2;
  static std::deque<int64_t> const offsetIncrs2;

  // 8 vectors; 5 states per deque; array length = 40
...
protected:
  std::shared_ptr<Lev1ParametricDescription> shared_from_this()
  {
    return std::static_pointer_cast<Lev1ParametricDescription>(

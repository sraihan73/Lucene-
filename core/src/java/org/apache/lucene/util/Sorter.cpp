...
{
  if (to < from) {
    throw invalid_argument(L"'to' must be >= 'from', got from=" +
                           to_wstring(from) + L" and to=" + to_wstring(to));
  }
}
...
void Sorter::mergeInPlace(int from, int mid, int to)
{
  ...
  constexpr int new_mid = first_cut + len22;
...
void Sorter::binarySort(int from, int to, int i)
{
  for (; i < to; ++i) {
    ...
    while (l <= h) {
      ...
      constexpr int cmp = comparePivot(mid);
      if (cmp < 0) {
        h = mid - 1;
      } else {
        l = mid + 1;
...
void Sorter::siftDown(int i, int from, int to)
{
  for (int leftChild = heapChild(from, i); leftChild < to;
    ...
    constexpr int rightChild = leftChild + 1;

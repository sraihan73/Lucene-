 ...
 * <p>
 * Consumers of this attribute call {@link #getBytesRef()} for each term.
 * Example: <pre class="prettyprint"> final TermToBytesRefAttribute termAtt =
 ...
 *
 *       // because the bytes are reused by the attribute (like
 * CharTermAttribute's char[] buffer),
...
{
  GET_CLASS_NAME(TermToBytesRefAttribute)

  /**
   * Retrieve this attribute's BytesRef. The bytes are updated from the current
   * term. The implementation may return a new instance or keep the previous
   * one.
   * @return a BytesRef to be indexed (only stays valid until token stream gets
   * incremented)
   */
public:
  virtual std::shared_ptr<BytesRef> getBytesRef() = 0;
};

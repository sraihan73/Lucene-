 ...
 * dimension representing a single float range, 2 dimensions representing a
 ...
 * operations over float ranges: <ul> <li>{@link #newIntersectsQuery
...
{
  GET_CLASS_NAME(FloatRange)
  /** stores float values so number of bytes is 4 */
public:
  static const int BYTES = Float::BYTES;
   ...
   */
  FloatRange(const std::wstring &name, std::deque<float> &min,
             std::deque<float> &max);
...
private:
   ...
   */
public:
  virtual void setRangeValues(std::deque<float> &min, std::deque<float> &max);

  /** validate the arguments */
private:
  static void checkArgs(std::deque<float> &min, std::deque<float> &max);

  /**
   * Encodes the min, max ranges into a byte array
   */
  static std::deque<char> encode(std::deque<float> &min,
                                  std::deque<float> &max);

  /**
   * encode the ranges into a sortable byte array ({@code Float.NaN} not
   * allowed) <p> example for 4 dimensions (8 bytes per dimension value): minD1
   * ... minD4 | maxD1 ... maxD4
   */
public:
  static void verifyAndEncode(std::deque<float> &min, std::deque<float> &max,
                              std::deque<char> &bytes);

  /** encode the given value into the byte array at the defined offset */
private:
  static void encode(float val, std::deque<char> &bytes, int offset);
   ...
   */
public:
  virtual float getMin(int dimension);
   ...
   */
  virtual float getMax(int dimension);

  /** decodes the min value (for the defined dimension) from the encoded input
   * byte array */
  static float decodeMin(std::deque<char> &b, int dimension);

  /** decodes the max value (for the defined dimension) from the encoded input
   * byte array */
  static float decodeMax(std::deque<char> &b, int dimension);
  ...
  /**
   * Create a query for matching indexed float ranges that contain the defined
   * range.
   * @param field field name. must not be null.
   * @param min array of min values. (accepts {@code Float.NEGATIVE_INFINITY})
   * @param max array of max values. (accepts {@code Float.POSITIVE_INFINITY})
...
protected:
  std::shared_ptr<FloatRange> shared_from_this()
  {
    return std::static_pointer_cast<FloatRange>(Field::shared_from_this());
  }
};

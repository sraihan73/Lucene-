 ...
 * dimension representing a single long range, 2 dimensions representing a
 ...
 * operations over long ranges: <ul> <li>{@link #newIntersectsQuery
...
public:
  static const int BYTES = Long::BYTES;
   ...
   */
  LongRange(const std::wstring &name, std::deque<int64_t> &min,
...
private:
   ...
   */
public:
  virtual void setRangeValues(std::deque<int64_t> &min,
                              std::deque<int64_t> &max);

  /** validate the arguments */
private:
  static void checkArgs(std::deque<int64_t> &min,
                        std::deque<int64_t> &max);

  /** Encodes the min, max ranges into a byte array */
  static std::deque<char> encode(std::deque<int64_t> &min,
                                  std::deque<int64_t> &max);
   ...
   */
public:
  static void verifyAndEncode(std::deque<int64_t> &min,
                              std::deque<int64_t> &max,
                              std::deque<char> &bytes);

  /** encode the given value into the byte array at the defined offset */
private:
  static void encode(int64_t val, std::deque<char> &bytes, int offset);
   ...
   */
public:
  virtual int64_t getMin(int dimension);
   ...
   */
  virtual int64_t getMax(int dimension);

  /** decodes the min value (for the defined dimension) from the encoded input
   * byte array */
  static int64_t decodeMin(std::deque<char> &b, int dimension);

  /** decodes the max value (for the defined dimension) from the encoded input
   * byte array */
  static int64_t decodeMax(std::deque<char> &b, int dimension);
...
protected:
  ...
  {
    return std::static_pointer_cast<LongRange>(Field::shared_from_this());
  }
};

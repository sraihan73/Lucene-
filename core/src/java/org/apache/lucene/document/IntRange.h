 ...
 * dimension representing a single integer range, 2 dimensions representing a
 ...
 * operations over integer ranges: <ul> <li>{@link #newIntersectsQuery
...
public:
   ...
   */
  IntRange(const std::wstring &name, std::deque<int> &min,
           std::deque<int> &max);
...
private:
   ...
   */
public:
  virtual void setRangeValues(std::deque<int> &min, std::deque<int> &max);

  /** validate the arguments */
private:
  static void checkArgs(std::deque<int> &min, std::deque<int> &max);

  /**
   * Encodes the min, max ranges into a byte array
   */
  static std::deque<char> encode(std::deque<int> &min, std::deque<int> &max);
   ...
   */
public:
  static void verifyAndEncode(std::deque<int> &min, std::deque<int> &max,
                              std::deque<char> &bytes);

  /** encode the given value into the byte array at the defined offset */
private:
  static void encode(int val, std::deque<char> &bytes, int offset);
   ...
   */
public:
  virtual int getMin(int dimension);
...
protected:
  std::shared_ptr<IntRange> shared_from_this()
  {
    return std::static_pointer_cast<IntRange>(Field::shared_from_this());
  }
};

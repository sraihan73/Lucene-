 ...
 * you will need to encode them with {@link
 * org.apache.lucene.util.NumericUtils}:
 *
 * <pre class="prettyprint">
 *   document.add(new SortedNumericDocValuesField(name,
 * NumericUtils.floatToSortableInt(-5.3f)));
 * </pre>
 *
 * <p>
...
{
  GET_CLASS_NAME(SortedNumericDocValuesField)
...
private:
  static SortedNumericDocValuesField::StaticConstructor staticConstructor;
   ...
   */
public:
  SortedNumericDocValuesField(const std::wstring &name, int64_t value);
   ...
   * Math.addExact(upperValue, -1)}. <p>This query also works with fields that
   * have indexed
...
protected:
  std::shared_ptr<SortedNumericDocValuesField> shared_from_this()
  {
    return std::static_pointer_cast<SortedNumericDocValuesField>(

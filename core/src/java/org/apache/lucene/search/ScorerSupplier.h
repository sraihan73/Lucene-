...
/**
 * A supplier of {@link Scorer}. This allows to get an estimate of the cost
 * before building the {@link Scorer}.
 */
class ScorerSupplier : public std::enable_shared_from_this<ScorerSupplier>
{
  GET_CLASS_NAME(ScorerSupplier)
   ...
   */
public:
  virtual std::shared_ptr<Scorer> get(int64_t leadCost) = 0;

  /**
   * Get an estimate of the {@link Scorer} that would be returned by {@link
   * #get}. This may be a costly operation, so it should only be called if
   * necessary.

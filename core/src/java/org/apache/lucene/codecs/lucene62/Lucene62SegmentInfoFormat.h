 ...
 *   <li>IsCompoundFile records whether the segment is written as a compound
 * file or not. If this is -1, the segment is not a compound file. If it is 1,
 * the segment is a compound file.</li> <li>The Diagnostics Map is privately
 * written by {@link IndexWriter}, as a debugging aid, for each segment it
 * creates. It includes metadata like the current Lucene version, OS, Java
 * version, why the segment was created (merge, flush, addIndexes), etc.</li>
 *   <li>Files is a deque of files referred to by this segment.</li>
 * </ul>
 *
 * @see SegmentInfos
 * @lucene.experimental
 */
class Lucene62SegmentInfoFormat : public SegmentInfoFormat
{
  GET_CLASS_NAME(Lucene62SegmentInfoFormat)
...
public:
  ...
  static constexpr int VERSION_MULTI_VALUED_SORT = 1;
  static constexpr int VERSION_CURRENT = VERSION_MULTI_VALUED_SORT;

protected:
  std::shared_ptr<Lucene62SegmentInfoFormat> shared_from_this()
  {
    return std::static_pointer_cast<Lucene62SegmentInfoFormat>(

...
#include "Lucene60FieldInfosFormat.h"
...
Lucene60FieldInfosFormat::Lucene60FieldInfosFormat() {}

shared_ptr<FieldInfos>
Lucene60FieldInfosFormat::read(shared_ptr<Directory> directory,
                               ...
                               shared_ptr<IOContext> context) 
{
  ...
  // input = directory.openChecksumInput(fileName, context))
  {
    ...
    try {
      CodecUtil::checkIndexHeader(input, Lucene60FieldInfosFormat::CODEC_NAME,
      ...
      for (int i = 0; i < size; i++) {
        ...
        bool isSoftDeletesField = (bits & SOFT_DELETES_FIELD) != 0;
...
Lucene60FieldInfosFormat::StaticConstructor::StaticConstructor()
{
...
Lucene60FieldInfosFormat::StaticConstructor
    Lucene60FieldInfosFormat::staticConstructor;

char Lucene60FieldInfosFormat::docValuesByte(DocValuesType type)
{
  switch (type) {
...
DocValuesType
Lucene60FieldInfosFormat::getDocValuesType(shared_ptr<IndexInput> input,
                                           char b) 
{
  switch (b) {
  case 0:
...
char Lucene60FieldInfosFormat::indexOptionsByte(IndexOptions indexOptions)
{
  switch (indexOptions) {
...
IndexOptions
Lucene60FieldInfosFormat::getIndexOptions(shared_ptr<IndexInput> input,
                                          char b) 
{
  switch (b) {
  case 0:
...
void Lucene60FieldInfosFormat::write(
    ...
    shared_ptr<IOContext> context) 
{
  ...
  // directory.createOutput(fileName, context))
  {
    org::apache::lucene::store::IndexOutput output =
        directory->createOutput(fileName, context);
    CodecUtil::writeIndexHeader(output, Lucene60FieldInfosFormat::CODEC_NAME,
                                Lucene60FieldInfosFormat::FORMAT_CURRENT,
                                segmentInfo->getId(), segmentSuffix);
...
const wstring Lucene60FieldInfosFormat::EXTENSION = L"fnm";
const wstring Lucene60FieldInfosFormat::CODEC_NAME = L"Lucene60FieldInfos";
} // namespace org::apache::lucene::codecs::lucene60
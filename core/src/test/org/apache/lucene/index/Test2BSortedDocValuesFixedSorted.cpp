...
#include "Test2BSortedDocValuesFixedSorted.h"
...
void Test2BSortedDocValuesFixedSorted::testFixedSorted() 
{
  shared_ptr<BaseDirectoryWrapper> dir =
      newFSDirectory(createTempDir(L"2BFixedSorted"));
  ...
  for (int i = 0; i < IndexWriter::MAX_DOCS; i++) {
    bytes[0] = static_cast<char>(i >> 8);
    bytes[1] = static_cast<char>(i);
    w->addDocument(doc);
    if (i % 100000 == 0) {
  ...
  for (shared_ptr<LeafReaderContext> context : r->leaves()) {
    ...
    for (int i = 0; i < reader->maxDoc(); i++) {
      ...
      bytes[0] = static_cast<char>(expectedValue >> 8);
      bytes[1] = static_cast<char>(expectedValue);

...
class CreateIndexTaskTest : public BenchmarkTestCase
{
  GET_CLASS_NAME(CreateIndexTaskTest)

private:
  std::shared_ptr<PerfRunData> createPerfRunData(
      const std::wstring &infoStreamValue) ;

public:
  virtual void testInfoStream_SystemOutErr() ;

  virtual void testInfoStream_File() ;

  virtual void testNoMergePolicy() ;

  virtual void testNoMergeScheduler() ;

  virtual void testNoDeletionPolicy() ;

protected:
  std::shared_ptr<CreateIndexTaskTest> shared_from_this()
  {
    return std::static_pointer_cast<CreateIndexTaskTest>(

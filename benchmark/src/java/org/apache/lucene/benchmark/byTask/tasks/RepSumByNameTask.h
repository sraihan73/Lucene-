 ...
 */
class RepSumByNameTask : public ReportTask
{
  GET_CLASS_NAME(RepSumByNameTask)

public:
  RepSumByNameTask(std::shared_ptr<PerfRunData> runData);
...
protected:
  ...
  reportSumByName(std::deque<std::shared_ptr<TaskStats>> &taskStats);

protected:
  std::shared_ptr<RepSumByNameTask> shared_from_this()
  {
    return std::static_pointer_cast<RepSumByNameTask>(

...
RepSumByNameTask::RepSumByNameTask(shared_ptr<PerfRunData> runData)
    : ReportTask(runData)
{
}

int RepSumByNameTask::doLogic() 
{
  shared_ptr<Report> rp =
      reportSumByName(getRunData()->getPoints()->taskStats());

  wcout << endl;
  wcout << L"------------> Report Sum By (any) Name (" << rp->getSize()
        << L" about " << rp->getReported() << L" out of " << rp->getOutOf()
        << L")" << endl;
...
shared_ptr<Report>
RepSumByNameTask::reportSumByName(deque<std::shared_ptr<TaskStats>> &taskStats)
{

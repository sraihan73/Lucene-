 ...
 */
class SearchTask : public ReadTask
{
  GET_CLASS_NAME(SearchTask)

public:
  SearchTask(std::shared_ptr<PerfRunData> runData);
...
protected:
  std::shared_ptr<SearchTask> shared_from_this()
  {
    return std::static_pointer_cast<SearchTask>(ReadTask::shared_from_this());
  }
};

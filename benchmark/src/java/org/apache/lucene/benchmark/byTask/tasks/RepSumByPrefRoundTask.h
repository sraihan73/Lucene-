 ...
 */
class RepSumByPrefRoundTask : public RepSumByPrefTask
{
  GET_CLASS_NAME(RepSumByPrefRoundTask)

public:
  RepSumByPrefRoundTask(std::shared_ptr<PerfRunData> runData);

  int doLogic()  override;

protected:
  virtual std::shared_ptr<Report>
  reportSumByPrefixRound(std::deque<std::shared_ptr<TaskStats>> &taskStats);

protected:
  std::shared_ptr<RepSumByPrefRoundTask> shared_from_this()
  {
    return std::static_pointer_cast<RepSumByPrefRoundTask>(

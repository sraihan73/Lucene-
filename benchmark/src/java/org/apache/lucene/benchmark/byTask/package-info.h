 ...
 *         <li><a href="#example">Example input algorithm and the result
 * benchmark report.</a></li> <li><a href="#recsCounting">Results record
 * counting clarified</a></li>
 *     </ol>
 * <a name="concept"></a>
 ...
 * "algorithm" file
 * that contains a description of the sequence of tasks making up the run, and
 * some properties defining a few additional characteristics of the benchmark
 * run.
 * </p>
 *
 * <a name="usage"></a>
 ...
 *      <br>- would run a performance test programmatically - without using an
 * alg file. This is less readable, and less convenient, but possible.
 *  </li>
 * </ul>
 *
 * <p>
 * You may find existing tasks sufficient for defining the benchmark <i>you</i>
 * need, otherwise, you can extend the framework to meet your needs, as
 * explained herein.
 * </p>
 *
 * <p>
 ...
 * used. You can also specify your own makers, extending DocMaker and
 * implementing QueryMaker. <blockquote> <b>Note:</b> since 2.9, DocMaker is a
 * concrete class which accepts a ContentSource. In most cases, you can use the
 * DocMaker class to create Documents, while providing your own ContentSource
 * implementation. For example, the current Benchmark package includes
 * ContentSource implementations for TREC, Enwiki and Reuters collections, as
 * well as others like LineDocSource which reads a 'line' file produced by
 *     WriteLineDocTask.
 *   </blockquote>
 *
 * <p>
 * Benchmark .alg file contains the benchmark "algorithm". The syntax is
 * described below. Within the algorithm, you can specify groups of commands,
 * assign them names, specify commands that should be repeated, do commands in
 * serial or in parallel, and also control the speed of "firing" the commands.
 * </p>
 *
 * <p>
 * This allows, for instance, to specify
 * that an index should be opened for update,
 * documents should be added to it one by one but not faster than 20 docs a
 * minute, and, in parallel with this, some N queries should be searched against
 * that index, again, no more than 2 queries a second. You can have the searches
 * all share an index reader, or have them each open its own reader and close it
 * afterwords.
 * </p>
 *
 * <p>
 ...
 *  </li>
 *  <li>
 *  <b>Serial</b> sequences are enclosed within '<span style="color: #FF0066">{
 * }</span>'.
 *  </li>
 *  <li>
 ...
 *  '<span style="color: #FF0066">{</span>' or '<span style="color:
 * #FF0066">[</span>'. <br>Example - <span style="color: #FF0066">{ "ManyAdds"
 * AddDoc } : 1000000</span> - would name the sequence of 1M add docs
 * "ManyAdds", and this name would later appear in statistic reports. If you
 * don't specify a name for a sequence, it is given one: you can see it as the
 * algorithm is printed just before benchmark execution starts.
 *  </li>
 *  <li>
 *  <b>Repeating</b>:
 *  To repeat sequence tasks N times, add '<span style="color: #FF0066">:
 * N</span>' just after the sequence closing tag - '<span style="color:
 * #FF0066">}</span>' or
 *  '<span style="color: #FF0066">]</span>' or '<span style="color:
 * #FF0066">&gt;</span>'. <br>Example -  <span style="color: #FF0066">[ AddDoc ]
 * : 4</span>  - would do 4 addDoc in parallel, spawning 4 threads at once.
 *  <br>Example -  <span style="color: #FF0066">[ AddDoc AddDoc ] : 4</span>  -
 * would do 8 addDoc in parallel, spawning 8 threads at once. <br>Example -
 * <span style="color: #FF0066">{ AddDoc } : 30</span> - would do addDoc 30
 * times in a row. <br>Example -  <span style="color: #FF0066">{ AddDoc AddDoc }
 * : 30</span> - would do addDoc 60 times in a row. <br><b>Exhaustive
 * repeating</b>: use <span style="color: #FF0066">*</span> instead of a number
 * to repeat exhaustively. This is sometimes useful, for adding as many files as
 * a doc maker can create, without iterating over the same file again,
 * especially when the exact number of documents is not known in advance. For
 * instance, TREC files extracted from a zip file. Note: when using this, you
 * must also set <span style="color: #FF0066">content.source.forever</span> to
 * false. <br>Example -  <span style="color: #FF0066">{ AddDoc } : *</span>  -
 * would add docs until the doc maker is "exhausted".
 *  </li>
 *  <li>
 *  <b>Command parameter</b>: a command can optionally take a single parameter.
 *  If the certain command does not support a parameter, or if the parameter is
 * of the wrong type, reading the algorithm will fail with an exception and the
 * test would not start. Currently the following tasks take optional parameters:
 *  <ul>
 *    <li><b>AddDoc</b> takes a numeric parameter, indicating the required size
 * of added document. Note: if the DocMaker implementation used in the test does
 * not support makeDoc(size), an exception would be thrown and the test would
 * fail.
 *    </li>
 ...
 *               parameter: a comma separated deque of Fields to load.
 *    </li>
 *    <li><b>SearchTravRetHighlighterTask</b> takes a string
 *               parameter: a comma separated deque of parameters to define
 * highlighting.  See that tasks javadocs for more information
 *    </li>
 *  </ul>
 *  <br>Example - <span style="color: #FF0066">AddDoc(2000)</span> - would add a
 * document of size 2000 (~bytes). <br>See conf/task-sample.alg for how this can
 * be used, for instance, to check which is faster, adding many smaller
 * documents, or few larger documents. Next candidates for supporting a
 ...
 *  <br>Example - <span style="color: #FF0066">{ "ManyAdds" AddDoc &gt; :
 * 1000000</span> - would add million docs, measure that total, but not save
 * stats for each addDoc. <br>Notice that the granularity of
 * System.currentTimeMillis() (which is used here) is system dependant, and in
 * some systems an operation that takes 5 ms to complete may show 0 ms latency
 * time in performance measurements. Therefore it is sometimes more accurate to
 * look at the elapsed time of a larger sequence, as demonstrated here.
 *  </li>
 *  <li>
 *  <b>Rate</b>:
 *  To set a rate (ops/sec or ops/min) for a sequence, add
 *  '<span style="color: #FF0066">: N : R</span>' just after sequence closing
 * tag. This would specify repetition of N with rate of R operations/sec. Use
 * '<span style="color: #FF0066">R/sec</span>' or
 ...
 *  <br>Example -  <span style="color: #FF0066">[ AddDoc ] : 400 : 3</span> -
 * would do 400 addDoc in parallel, starting up to 3 threads per second.
 *  <br>Example -  <span style="color: #FF0066">{ AddDoc } : 100 :
 * 200/min</span> - would do 100 addDoc serially, waiting before starting next
 * add, if otherwise rate would exceed 200 adds/min.
 *  </li>
 *  <li>
 *  <b>Disable Counting</b>: Each task executed contributes to the records
 * count. This count is reflected in reports under recs/s and under recsPerRun.
 *  Most tasks count 1, some count 0, and some count more.
 *  (See <a href="#recsCounting">Results record counting clarified</a> for more
 * details.) It is possible to disable counting for a task by preceding it with
 * <span style="color: #FF0066">-</span>. <br>Example -  <span style="color:
 * #FF0066"> -CreateIndex </span> - would count 0 while the default behavior for
 * CreateIndex is to count 1.
 *  </li>
 *  <li>
 ...
 *  <ul>
 *              <li>
 *             <span style="color: #FF0066">RepAll</span> - all (completed) task
 * runs.
 *             </li>
 *             <li>
 *             <span style="color: #FF0066">RepSumByName</span> - all
 * statistics, aggregated by name. So, if AddDoc was executed 2000 times, only 1
 * report line would be created for it, aggregating all those 2000 statistic
 * records.
 *             </li>
 *             <li>
 *             <span style="color: #FF0066">RepSelectByPref &nbsp;
 * prefixWord</span> - all records for tasks whose name start with <span
 * style="color: #FF0066">prefixWord</span>.
 *             </li>
 *             <li>
 *             <span style="color: #FF0066">RepSumByPref &nbsp;
 * prefixWord</span> - all records for tasks whose name start with <span
 * style="color: #FF0066">prefixWord</span>, aggregated by their full task name.
 *             </li>
 *             <li>
 *             <span style="color: #FF0066">RepSumByNameRound</span> - all
 * statistics, aggregated by name and by <span style="color:
 * #FF0066">Round</span>. So, if AddDoc was executed 2000 times in each of 3
 *             <span style="color: #FF0066">rounds</span>, 3 report lines would
 * be created for it, aggregating all those 2000 statistic records in each
 * round. See more about rounds in the <span style="color:
 * #FF0066">NewRound</span> command description below.
 *             </li>
 *             <li>
 *             <span style="color: #FF0066">RepSumByPrefRound &nbsp;
 * prefixWord</span> - similar to <span style="color:
 * #FF0066">RepSumByNameRound</span>, just that only tasks whose name starts
 * with <span style="color: #FF0066">prefixWord</span> are included.
 *             </li>
 *  </ul>
 ...
 *  <ul>
 *      <li>
 *      <span style="color: #FF0066">ClearStats</span> - clears the entire
 * statistics. Further reports would only include task runs that would start
 * after this call.
 *      </li>
 *      <li>
 *      <span style="color: #FF0066">NewRound</span> - virtually start a new
 * round of performance test. Although this command can be placed anywhere, it
 * mostly makes sense at the end of an outermost sequence. <br>This increments a
 * global "round counter". All task runs that would start now would record the
 * new, updated round counter as their round number. This would appear in
 * reports. In particular, see <span style="color:
 * #FF0066">RepSumByNameRound</span> above. <br>An additional effect of
 * NewRound, is that numeric and bool properties defined (at the head of the
 * .alg file) as a sequence of values, e.g. <span style="color: #FF0066">
 ...
 * numeric and bool properties that are defined as a sequence
 * of values, e.g. <span style="color:
 * #FF0066">merge.factor=mrg:10:100:10:100</span> would increment (cyclic) to
 * the next value, when NewRound is called, and would also appear as a named
 * column in the reports (column name would be "mrg" in this example).
 * </p>
 *
 * <p>
 ...
 *     Multi int/bool values would be iterated with calls to NewRound.
 ...
 *         using the compound format or not. Valid values are "true" and
 * "false".
 *         </li>
 *     </ul>
 * </ol>
 *
 * <p>
 * Here is a deque of currently defined properties:
 * </p>
 * <ol>
 *
 *   <li><b>Root directory for data and indexes:</b>
 *     <ul><li>work.dir (default is System property "benchmark.work.dir" or
 * "work".)
 *     </li></ul>
 *   </li>
 *
 ...
 *     </li><li>doc.stored
 *     </li><li>doc.tokenized
 *     </li><li>doc.term.deque
 *     </li><li>doc.term.deque.positions
 *     </li><li>doc.term.deque.offsets
 ...
 *   <li><b>Logging</b>:
 *     <ul><li>log.step
 *   </li><li>log.step.[class name]Task ie log.step.DeleteDoc (e.g.
 * log.step.Wonderful for the WonderfulTask example above).
 *     </li><li>log.queries
 ...
 *     </li><li>directory
 *     </li><li>ram.flush.mb
 *     </li><li>codec.postingsFormat (eg Direct) Note: no codec should be
 * specified through default.codec
 *     </li></ul>
 *   </li>
 *
 *   <li><b>Doc deletion</b>:
 ...
 *       - comma separated deque of additional packages where tasks classes will
 * be looked for when not found in the default package (that of PerfTask).  If
 * the same task class appears in more than one package, the package indicated
 * first in this deque will be used.
 *     </li></ul>
 *   </li>
 *
 * </ol>
 ...
 * column name. merge.factor=mrg:10:20 max.buffered=buf:100:1000 compound=true
 *
 ...
 * avgUsedMem    avgTotalMem PopulateShort     0  10  100        1        20003
 * 119.6      167.26    12,959,120     14,241,792 PopulateLong -  - 0  10  100 -
 * -   1 -  -   10003 -  -  - 74.3 -  - 134.57 -  17,085,208 -   20,635,648
 * PopulateShort     1  20 1000        1        20003        143.5      139.39
 * 63,982,040     94,756,864 PopulateLong -  - 1  20 1000 -  -   1 -  -   10003
 * -  -  - 77.0 -  - 129.92 -  87,309,608 -  100,831,232
 * </pre>
 *
 ...
 * </p><p>
 * Confusing? this might help: always examine the <code>elapsedSec</code>
 * column, and always compare "apples to apples", .i.e. it is interesting to
 * check how the <code>rec/s</code> changed for the same task (or sequence)
 * between two different runs, but it is not very useful to know how the
 * <code>rec/s</code> differs between <code>Search</code> and
 * <code>SearchTrav</code> tasks. For the latter, <code>elapsedSec</code> would
 * bring more insight.
 * </p>
 */

 ...
 */
class OpenTaxonomyReaderTask : public PerfTask
{
  GET_CLASS_NAME(OpenTaxonomyReaderTask)

public:
  OpenTaxonomyReaderTask(std::shared_ptr<PerfRunData> runData);

  int doLogic()  override;

protected:
  std::shared_ptr<OpenTaxonomyReaderTask> shared_from_this()
  {
    return std::static_pointer_cast<OpenTaxonomyReaderTask>(

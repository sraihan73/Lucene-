 ...
 */
class CreateTaxonomyIndexTask : public PerfTask
{
  GET_CLASS_NAME(CreateTaxonomyIndexTask)

public:
  CreateTaxonomyIndexTask(std::shared_ptr<PerfRunData> runData);

  int doLogic()  override;

protected:
  std::shared_ptr<CreateTaxonomyIndexTask> shared_from_this()
  {
    return std::static_pointer_cast<CreateTaxonomyIndexTask>(

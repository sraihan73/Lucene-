 ...
 */
class CloseTaxonomyReaderTask : public PerfTask
{
  GET_CLASS_NAME(CloseTaxonomyReaderTask)

public:
  CloseTaxonomyReaderTask(std::shared_ptr<PerfRunData> runData);

  int doLogic()  override;

protected:
  std::shared_ptr<CloseTaxonomyReaderTask> shared_from_this()
  {
    return std::static_pointer_cast<CloseTaxonomyReaderTask>(

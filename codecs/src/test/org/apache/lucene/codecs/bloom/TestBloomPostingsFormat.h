#pragma once
#include "../../../../../../../../test-framework/src/java/org/apache/lucene/codecs/bloom/TestBloomFilteredLucenePostings.h"
 ...
 */
class TestBloomPostingsFormat : public BasePostingsFormatTestCase
{
  GET_CLASS_NAME(TestBloomPostingsFormat)
private:
  const std::shared_ptr<Codec> codec = TestUtil::alwaysPostingsFormat(
      std::make_shared<TestBloomFilteredLucenePostings>());

protected:
  std::shared_ptr<Codec> getCodec() override;

protected:
  std::shared_ptr<TestBloomPostingsFormat> shared_from_this()
  {
    return std::static_pointer_cast<TestBloomPostingsFormat>(

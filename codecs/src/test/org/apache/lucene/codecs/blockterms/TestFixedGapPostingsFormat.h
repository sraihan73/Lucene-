#pragma once
#include "../../../../../../../../test-framework/src/java/org/apache/lucene/codecs/blockterms/LuceneFixedGap.h"
 ...
 */
class TestFixedGapPostingsFormat : public BasePostingsFormatTestCase
{
  GET_CLASS_NAME(TestFixedGapPostingsFormat)
private:
  const std::shared_ptr<Codec> codec = TestUtil::alwaysPostingsFormat(
      std::make_shared<LuceneFixedGap>(TestUtil::nextInt(random(), 1, 1000)));

protected:
  std::shared_ptr<Codec> getCodec() override;

protected:
  std::shared_ptr<TestFixedGapPostingsFormat> shared_from_this()
  {
    return std::static_pointer_cast<TestFixedGapPostingsFormat>(

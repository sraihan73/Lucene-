...
#include "../../../../../../java/org/apache/lucene/codecs/memory/FSTPostingsFormat.h"
 ...
 */
class TestFSTPostingsFormat : public BasePostingsFormatTestCase
{
  GET_CLASS_NAME(TestFSTPostingsFormat)
private:
  const std::shared_ptr<Codec> codec =
      TestUtil::alwaysPostingsFormat(std::make_shared<FSTPostingsFormat>());

protected:
  std::shared_ptr<Codec> getCodec() override;

protected:
  std::shared_ptr<TestFSTPostingsFormat> shared_from_this()
  {
    return std::static_pointer_cast<TestFSTPostingsFormat>(

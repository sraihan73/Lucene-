...
#include "../../../../../../java/org/apache/lucene/codecs/memory/DirectPostingsFormat.h"
 ...
 */
class TestDirectPostingsFormat : public BasePostingsFormatTestCase
{
  GET_CLASS_NAME(TestDirectPostingsFormat)
  // TODO: randomize parameters
private:
  const std::shared_ptr<Codec> codec =
      TestUtil::alwaysPostingsFormat(std::make_shared<DirectPostingsFormat>());
...
protected:
  std::shared_ptr<TestDirectPostingsFormat> shared_from_this()
  {
    return std::static_pointer_cast<TestDirectPostingsFormat>(

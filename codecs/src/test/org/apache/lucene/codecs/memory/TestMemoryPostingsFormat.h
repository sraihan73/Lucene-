...
#include "../../../../../../java/org/apache/lucene/codecs/memory/MemoryPostingsFormat.h"
 ...
 */
class TestMemoryPostingsFormat : public BasePostingsFormatTestCase
{
  GET_CLASS_NAME(TestMemoryPostingsFormat)
  // TODO: randomize doPack
private:
  const std::shared_ptr<Codec> codec =
      TestUtil::alwaysPostingsFormat(std::make_shared<MemoryPostingsFormat>());

protected:
  std::shared_ptr<Codec> getCodec() override;

protected:
  std::shared_ptr<TestMemoryPostingsFormat> shared_from_this()
  {
    return std::static_pointer_cast<TestMemoryPostingsFormat>(

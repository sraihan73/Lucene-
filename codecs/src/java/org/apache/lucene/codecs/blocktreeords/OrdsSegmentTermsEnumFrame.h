...
class OrdsSegmentTermsEnumFrame final
    : public std::enable_shared_from_this<OrdsSegmentTermsEnumFrame>
{
  GET_CLASS_NAME(OrdsSegmentTermsEnumFrame)
  // Our index in stack[]:
public:
  const int ord;
  ...
  // static bool DEBUG = OrdsSegmentTermsEnum.DEBUG;
...
private:
  const std::shared_ptr<OrdsSegmentTermsEnum> ste;

public:
  OrdsSegmentTermsEnumFrame(std::shared_ptr<OrdsSegmentTermsEnum> ste,
                            int ord) ;
  ...
  // likely not worth it?  need to measure how many
  // floor blocks we "typically" get
  void scanToFloorFrame(int64_t targetOrd);

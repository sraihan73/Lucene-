...
class OrdsBlockTreeTermsWriter final : public FieldsConsumer
{
  GET_CLASS_NAME(OrdsBlockTreeTermsWriter)

public:
  static const std::shared_ptr<FSTOrdsOutputs> FST_OUTPUTS;
...
private:
  ...
  public:
    ...
    FieldMetaData(std::shared_ptr<FieldInfo> fieldInfo,
                  std::shared_ptr<Output> rootCode, int64_t numTerms,
...
public:
  OrdsBlockTreeTermsWriter(std::shared_ptr<SegmentWriteState> state,
...
private:
  class SubIndex final : public std::enable_shared_from_this<SubIndex>
  {
    GET_CLASS_NAME(SubIndex)
  public:
    const std::shared_ptr<FST<std::shared_ptr<Output>>> index;
    const int64_t termOrdStart;

    SubIndex(std::shared_ptr<FST<std::shared_ptr<Output>>> index,
             int64_t termOrdStart);
  };
...
private:
  ...
  public:
    ...
    const int64_t fp;
    std::shared_ptr<FST<std::shared_ptr<Output>>> index;
    std::deque<std::shared_ptr<SubIndex>> subIndices;
    const bool hasTerms;
    const bool isFloor;
    const int floorLeadByte;
    int64_t totFloorTermCount = 0;

  private:
    const int64_t totalTermCount;
  ...
  private:
    void
    append(std::shared_ptr<Builder<std::shared_ptr<Output>>> builder,
           std::shared_ptr<FST<std::shared_ptr<Output>>> subIndex,
           int64_t termOrdOffset,
           std::shared_ptr<IntsRefBuilder> scratchIntsRef) ;
...
public:
  ...
  private:
    std::shared_ptr<OrdsBlockTreeTermsWriter> outerInstance;
  ...
  public:
    TermsWriter(std::shared_ptr<OrdsBlockTreeTermsWriter> outerInstance,
...
public:
  virtual ~OrdsBlockTreeTermsWriter();
...
protected:
  std::shared_ptr<OrdsBlockTreeTermsWriter> shared_from_this()
  {
    return std::static_pointer_cast<OrdsBlockTreeTermsWriter>(

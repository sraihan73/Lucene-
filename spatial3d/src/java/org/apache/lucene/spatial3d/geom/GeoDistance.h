...
/**
 * An implementer of this interface is capable of computing the described
 * "distance" values, which are meant to provide both actual distance values, as
 * well as distance estimates that can be computed more cheaply.
 *
 * @lucene.experimental
 */
class GeoDistance : public Membership
{
  GET_CLASS_NAME(GeoDistance)

  // The following methods compute distances from the shape to a point
  // expected to be INSIDE the shape.  Typically a value of
  // Double.POSITIVE_INFINITY is returned for points that happen to be outside
  // the shape.
  ...
  //      public default double computeDistance(final DistanceStyle
   ...
   */
public:
  virtual double computeDistance(std::shared_ptr<DistanceStyle> distanceStyle,
                                 double const x, double const y,
                                 double const z) = 0;

  /**
   * Compute the shape's <em>delta</em> distance given a point.  This is defined
   * as the distance that someone traveling the "length" of the shape would have
   * to go out of their way to include the point. For some shapes, e.g. paths,
   * this makes perfect sense.  For other shapes, e.g. circles, the "length" of
   * the shape is zero, and the delta is computed as the distance from the
   * center to the point and back. A return value of Double.POSITIVE_INFINITY

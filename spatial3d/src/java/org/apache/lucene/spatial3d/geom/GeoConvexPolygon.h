...
/**
 * GeoConvexPolygon objects are generic building blocks of more complex
 * structures. The only restrictions on these objects are: (1) they must be
 * convex; (2) they must have a maximum extent no larger than PI.  Violating
 * either one of these limits will cause the logic to fail.
 *
 * @lucene.internal
 */
class GeoConvexPolygon : public GeoBasePolygon
{
  GET_CLASS_NAME(GeoConvexPolygon)
...
protected:
  ...
  /**
   * Create a convex polygon from a deque of points.  The first point must be on
   *the external edge.
   *@param planetModel is the planet model.
   *@param pointList is the deque of points to create the polygon from.
   */
public:
  GeoConvexPolygon(std::shared_ptr<PlanetModel> planetModel,
  ...
  /**
   * Create a convex polygon from a deque of points, keeping track of which
...
public:
  ...
  bool intersects(std::shared_ptr<GeoShape> shape) override;
...
protected:
  std::shared_ptr<GeoConvexPolygon> shared_from_this()
  {
    return std::static_pointer_cast<GeoConvexPolygon>(

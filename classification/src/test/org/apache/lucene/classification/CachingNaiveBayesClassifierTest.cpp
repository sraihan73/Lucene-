...
#include "CachingNaiveBayesClassifierTest.h"
...
void CachingNaiveBayesClassifierTest::testBasicUsage() 
{
  ...
  try {
    ...
    checkCorrectClassification(
        make_shared<CachingNaiveBayesClassifier>(
            leafReader, analyzer, nullptr, categoryFieldName, textFieldName),
        POLITICS_INPUT, POLITICS_RESULT);
  }
...
void CachingNaiveBayesClassifierTest::testBasicUsageWithQuery() throw(
    runtime_error)
{
  shared_ptr<LeafReader> leafReader = nullptr;
  try {
...
void CachingNaiveBayesClassifierTest::testNGramUsage() 
{
  shared_ptr<LeafReader> leafReader = nullptr;
  try {
    shared_ptr<NGramAnalyzer> analyzer = make_shared<NGramAnalyzer>();
    leafReader = getSampleIndex(analyzer);
    checkCorrectClassification(
        make_shared<CachingNaiveBayesClassifier>(
            leafReader, analyzer, nullptr, categoryFieldName, textFieldName),
...
CachingNaiveBayesClassifierTest::NGramAnalyzer::createComponents(
...
void CachingNaiveBayesClassifierTest::testPerformance() 
{
  shared_ptr<MockAnalyzer> analyzer = make_shared<MockAnalyzer>(random());
  shared_ptr<LeafReader> leafReader = getRandomIndex(analyzer, 100);
  try {
    shared_ptr<CachingNaiveBayesClassifier> simpleNaiveBayesClassifier =
        make_shared<CachingNaiveBayesClassifier>(
            leafReader, analyzer, nullptr, categoryFieldName, textFieldName);

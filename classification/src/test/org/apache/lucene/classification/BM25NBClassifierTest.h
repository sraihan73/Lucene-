 ...
 */
class BM25NBClassifierTest
    : public ClassificationTestBase<std::shared_ptr<BytesRef>>
{
  GET_CLASS_NAME(BM25NBClassifierTest)
...
private:
  ...
  private:
    std::shared_ptr<BM25NBClassifierTest> outerInstance;

  public:
    NGramAnalyzer(std::shared_ptr<BM25NBClassifierTest> outerInstance);
...
protected:
  std::shared_ptr<BM25NBClassifierTest> shared_from_this()
  {
    return std::static_pointer_cast<BM25NBClassifierTest>(

 ...
 */
class SpanNotBuilder : public SpanBuilderBase
{
  GET_CLASS_NAME(SpanNotBuilder)

private:
  const std::shared_ptr<SpanQueryBuilder> factory;

public:
  SpanNotBuilder(std::shared_ptr<SpanQueryBuilder> factory);

  std::shared_ptr<SpanQuery>
  getSpanQuery(std::shared_ptr<Element> e)  override;

protected:
  std::shared_ptr<SpanNotBuilder> shared_from_this()
  {
    return std::static_pointer_cast<SpanNotBuilder>(

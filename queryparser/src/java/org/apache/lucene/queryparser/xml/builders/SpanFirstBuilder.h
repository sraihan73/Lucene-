...
#include  "core/src/java/org/apache/lucene/queryparser/xml/builders/SpanQueryBuilder.h"
 ...
 */
class SpanFirstBuilder : public SpanBuilderBase
{
  GET_CLASS_NAME(SpanFirstBuilder)

private:
  const std::shared_ptr<SpanQueryBuilder> factory;

public:
  SpanFirstBuilder(std::shared_ptr<SpanQueryBuilder> factory);

  std::shared_ptr<SpanQuery>
  getSpanQuery(std::shared_ptr<Element> e)  override;

protected:
  std::shared_ptr<SpanFirstBuilder> shared_from_this()
  {
    return std::static_pointer_cast<SpanFirstBuilder>(

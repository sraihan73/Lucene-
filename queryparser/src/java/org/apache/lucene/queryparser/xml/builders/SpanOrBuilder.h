 ...
 */
class SpanOrBuilder : public SpanBuilderBase
{
  GET_CLASS_NAME(SpanOrBuilder)

private:
  const std::shared_ptr<SpanQueryBuilder> factory;

public:
  SpanOrBuilder(std::shared_ptr<SpanQueryBuilder> factory);

  std::shared_ptr<SpanQuery>
  getSpanQuery(std::shared_ptr<Element> e)  override;

protected:
  std::shared_ptr<SpanOrBuilder> shared_from_this()
  {
    return std::static_pointer_cast<SpanOrBuilder>(

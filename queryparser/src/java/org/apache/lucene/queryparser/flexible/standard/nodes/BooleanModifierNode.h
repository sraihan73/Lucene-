 ...
 */
class BooleanModifierNode : public ModifierQueryNode
{
  GET_CLASS_NAME(BooleanModifierNode)

public:
  BooleanModifierNode(std::shared_ptr<QueryNode> node, Modifier mod);

protected:
  std::shared_ptr<BooleanModifierNode> shared_from_this()
  {
    return std::static_pointer_cast<BooleanModifierNode>(
        org.apache.lucene.queryparser.flexible.core.nodes
            .ModifierQueryNode::shared_from_this());
  }
};

...
class SynonymQueryNode : public BooleanQueryNode
{
  GET_CLASS_NAME(SynonymQueryNode)
  /** Sole constructor. */
public:
  SynonymQueryNode(std::deque<std::shared_ptr<QueryNode>> &clauses);

protected:
  std::shared_ptr<SynonymQueryNode> shared_from_this()
  {
    return std::static_pointer_cast<SynonymQueryNode>(

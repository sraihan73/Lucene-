 ...
 */
class AndQuery : public ComposedQuery
{
  GET_CLASS_NAME(AndQuery)
public:
  AndQuery(std::deque<std::shared_ptr<SrndQuery>> &queries, bool inf,
...
protected:
  std::shared_ptr<AndQuery> shared_from_this()
  {
    return std::static_pointer_cast<AndQuery>(

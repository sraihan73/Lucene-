...
class TestParallelDrillSideways : public TestDrillSideways
{
  GET_CLASS_NAME(TestParallelDrillSideways)

private:
  static std::shared_ptr<ExecutorService> executor;

public:
  // C++ TODO: Most Java annotations will not have direct C++ equivalents:
  // ORIGINAL LINE: @BeforeClass public static void prepareExecutor()
  static void prepareExecutor();
...
protected:
  std::shared_ptr<DrillSideways> getNewDrillSideways(
      std::shared_ptr<IndexSearcher> searcher,
      std::shared_ptr<FacetsConfig> config,
      std::shared_ptr<SortedSetDocValuesReaderState> state) override;

  std::shared_ptr<DrillSideways>
  getNewDrillSideways(std::shared_ptr<IndexSearcher> searcher,
                      std::shared_ptr<FacetsConfig> config,
                      std::shared_ptr<TaxonomyReader> taxoReader) override;

  std::shared_ptr<DrillSideways> getNewDrillSidewaysScoreSubdocsAtOnce(
      std::shared_ptr<IndexSearcher> searcher,
      std::shared_ptr<FacetsConfig> config,
      std::shared_ptr<TaxonomyReader> taxoReader) override;
...
private:
  ...
  private:
    std::shared_ptr<TestParallelDrillSideways> outerInstance;
...
protected:
  std::shared_ptr<DrillSideways> getNewDrillSidewaysBuildFacetsResult(
      std::shared_ptr<IndexSearcher> searcher,
      std::shared_ptr<FacetsConfig> config,
      std::shared_ptr<TaxonomyReader> taxoReader) override;
...
protected:
  std::shared_ptr<TestParallelDrillSideways> shared_from_this()
  {
    return std::static_pointer_cast<TestParallelDrillSideways>(
        TestDrillSideways::shared_from_this());
  }
};

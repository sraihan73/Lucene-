...
#include "ConcurrentSortedSetDocValuesFacetCounts.h"
...
ConcurrentSortedSetDocValuesFacetCounts::
    ConcurrentSortedSetDocValuesFacetCounts(
        ...
        shared_ptr<ExecutorService> exec) throw(IOException,
                                                InterruptedException)
    : ConcurrentSortedSetDocValuesFacetCounts(state, nullptr, exec)
{
}
...
ConcurrentSortedSetDocValuesFacetCounts::
    ConcurrentSortedSetDocValuesFacetCounts(
        ...
        shared_ptr<ExecutorService> exec) throw(IOException,
                                                InterruptedException)
    : exec(exec), state(state), dv(state->getDocValues()),
      field(state->getField()),
      counts(make_shared<AtomicIntegerArray>(state->getSize()))
{
  if (hits == nullptr) {
    // browse only
    countAll();
...
shared_ptr<FacetResult> ConcurrentSortedSetDocValuesFacetCounts::getTopChildren(
...
ConcurrentSortedSetDocValuesFacetCounts::getDim(const wstring &dim,
...
ConcurrentSortedSetDocValuesFacetCounts::CountOneSegment::CountOneSegment(
    shared_ptr<ConcurrentSortedSetDocValuesFacetCounts> outerInstance,
    shared_ptr<LeafReader> leafReader, shared_ptr<MatchingDocs> hits,
    shared_ptr<OrdinalMap> ordinalMap, int segOrd)
    : leafReader(leafReader), hits(hits), ordinalMap(ordinalMap),
      segOrd(segOrd), outerInstance(outerInstance)
{
}

shared_ptr<Void>
ConcurrentSortedSetDocValuesFacetCounts::CountOneSegment::call() throw(
    IOException)
{
  shared_ptr<SortedSetDocValues> segValues =
      leafReader->getSortedSetDocValues(outerInstance->field);
  ...
  if (ordinalMap != nullptr) {
    ...
    if (hits != nullptr && hits->totalHits < numSegOrds / 10) {
      ...
      for (int doc = it->nextDoc(); doc != DocIdSetIterator::NO_MORE_DOCS;
        ...
        while (term != SortedSetDocValues::NO_MORE_ORDS) {
          outerInstance->counts->incrementAndGet(
              static_cast<int>(ordMap->get(term)));
      ...
      for (int ord = 0; ord < numSegOrds; ord++) {
        ...
        if (count != 0) {
          outerInstance->counts->addAndGet(static_cast<int>(ordMap->get(ord)),
                                           count);
        }
      }
    }
    ...
    for (int doc = it->nextDoc(); doc != DocIdSetIterator::NO_MORE_DOCS;
      ...
      int term = static_cast<int>(segValues->nextOrd());
      while (term != SortedSetDocValues::NO_MORE_ORDS) {
        outerInstance->counts->incrementAndGet(term);
        term = static_cast<int>(segValues->nextOrd());
      }
    }
  }
...
void ConcurrentSortedSetDocValuesFacetCounts::count(
        ...
        &matchingDocs) 
{
  ...
  deque<Future<std::shared_ptr<Void>>> results =
      deque<Future<std::shared_ptr<Void>>>();
  ...
  for (auto hits : matchingDocs) {
    ...
    results.push_back(exec->submit(make_shared<CountOneSegment>(
        shared_from_this(), hits->context->reader(), hits, ordinalMap,
        hits->context->ord)));
  }
...
void ConcurrentSortedSetDocValuesFacetCounts::countAll() throw(
    IOException, InterruptedException)
{
  ...
  for (auto context : state->getReader()->leaves()) {
    results.push_back(exec->submit(
        make_shared<CountOneSegment>(shared_from_this(), context->reader(),
                                     nullptr, ordinalMap, context->ord)));
  }
...
shared_ptr<Number> ConcurrentSortedSetDocValuesFacetCounts::getSpecificValue(
...
ConcurrentSortedSetDocValuesFacetCounts::ComparatorAnonymousInnerClass::
...
int ConcurrentSortedSetDocValuesFacetCounts::ComparatorAnonymousInnerClass::
    compare(shared_ptr<FacetResult> a, shared_ptr<FacetResult> b)
{

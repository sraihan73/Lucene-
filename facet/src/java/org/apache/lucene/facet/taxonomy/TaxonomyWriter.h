...
using TwoPhaseCommit = org::apache::lucene::index::TwoPhaseCommit;
 ...
 * @lucene.experimental
 */
class TaxonomyWriter : public Closeable, public TwoPhaseCommit
{
  GET_CLASS_NAME(TaxonomyWriter)
   ...
   */
public:
  virtual int addCategory(std::shared_ptr<FacetLabel> categoryPath) = 0;
   ...
   * exactly the same code and cache used by addCategory() so it means less
   * code.
   */
  virtual int getParent(int ordinal) = 0;
  ...
  /**
   * Sets the commit user data iterable.  See {@link
   * IndexWriter#setLiveCommitData}.
   */
  virtual void setLiveCommitData(
   ...
   */
  virtual std::deque<std::unordered_map::Entry<std::wstring, std::wstring>>
  getLiveCommitData() = 0;
};

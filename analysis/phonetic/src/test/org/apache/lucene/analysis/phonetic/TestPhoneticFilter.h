 ...
 */
class TestPhoneticFilter : public BaseTokenStreamTestCase
{
  GET_CLASS_NAME(TestPhoneticFilter)

public:
  virtual void testAlgorithms() ;

  static void assertAlgorithm(
      std::shared_ptr<Encoder> encoder, bool inject, const std::wstring &input,
...
private:
  ...
  private:
    std::shared_ptr<TestPhoneticFilter> outerInstance;
...
protected:
  std::shared_ptr<TestPhoneticFilter> shared_from_this()
  {
    return std::static_pointer_cast<TestPhoneticFilter>(

...
class TestDoubleMetaphoneFilterFactory : public BaseTokenStreamTestCase
{
  GET_CLASS_NAME(TestDoubleMetaphoneFilterFactory)

public:
  virtual void testDefaults() ;

  virtual void testSettingSizeAndInject() ;
...
private:
  ...
  private:
    std::shared_ptr<TestDoubleMetaphoneFilterFactory> outerInstance;
...
protected:
  std::shared_ptr<TestDoubleMetaphoneFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<TestDoubleMetaphoneFilterFactory>(

 ...
 */
class TestDaitchMokotoffSoundexFilter : public BaseTokenStreamTestCase
{
  GET_CLASS_NAME(TestDaitchMokotoffSoundexFilter)

public:
  virtual void testAlgorithms() ;

  static void assertAlgorithm(
      bool inject, const std::wstring &input,
...
private:
  ...
  private:
    std::shared_ptr<TestDaitchMokotoffSoundexFilter> outerInstance;
...
protected:
  std::shared_ptr<TestDaitchMokotoffSoundexFilter> shared_from_this()
  {
    return std::static_pointer_cast<TestDaitchMokotoffSoundexFilter>(

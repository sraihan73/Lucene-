...
TokenInfoFST::TokenInfoFST(shared_ptr<FST<int64_t>> fst,
    ...
    : fst(fst), cacheCeiling(fasterButMoreRam ? 0x9FFF : 0x30FF),
      rootCache(cacheRootArcs()), NO_OUTPUT(fst->outputs->getNoOutput())
{
}
...
std::deque<Arc<int64_t>> TokenInfoFST::cacheRootArcs() 
{
  std::deque<Arc<int64_t>> rootCache =
      std::deque<std::shared_ptr<Arc>>(1 + (cacheCeiling - 0x3040));
  ...
  for (int i = 0; i < rootCache.size(); i++) {
    if (fst->findTargetArc(0x3040 + i, firstArc, arc, fstReader) != nullptr) {
    ...
    bool useCache, shared_ptr<FST::BytesReader> fstReader) 
{
  if (useCache && ch >= 0x3040 && ch <= cacheCeiling) {
    ...
    shared_ptr<Arc<int64_t>> *const result = rootCache[ch - 0x3040];
    if (result == nullptr) {
      return nullptr;
    } else {
      arc->copyFrom(result);

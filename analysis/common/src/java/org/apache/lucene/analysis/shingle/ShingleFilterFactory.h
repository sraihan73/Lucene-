 ...
 * class="solr.ShingleFilterFactory" minShingleSize="2" maxShingleSize="2"
 *             outputUnigrams="true" outputUnigramsIfNoShingles="false"
 * tokenSeparator=" " fillerToken="_"/&gt; &lt;/analyzer&gt;
 * &lt;/fieldType&gt;</pre>
 */
class ShingleFilterFactory : public TokenFilterFactory
{
  GET_CLASS_NAME(ShingleFilterFactory)
private:
...
public:
  ShingleFilterFactory(std::unordered_map<std::wstring, std::wstring> &args);

  std::shared_ptr<ShingleFilter>
  create(std::shared_ptr<TokenStream> input) override;

protected:
  std::shared_ptr<ShingleFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<ShingleFilterFactory>(

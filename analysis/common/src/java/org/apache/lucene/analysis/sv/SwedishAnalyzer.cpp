...
#include "../../../../tartarus/snowball/ext/SwedishStemmer.h"
...
const wstring SwedishAnalyzer::DEFAULT_STOPWORD_FILE = L"swedish_stop.txt";

shared_ptr<CharArraySet> SwedishAnalyzer::getDefaultStopSet()
{
...
SwedishAnalyzer::DefaultSetHolder::StaticConstructor::StaticConstructor()
{
  try {
    DEFAULT_STOP_SET =
...
DefaultSetHolder::StaticConstructor
    SwedishAnalyzer::DefaultSetHolder::staticConstructor;
...
SwedishAnalyzer::SwedishAnalyzer(shared_ptr<CharArraySet> stopwords)
    : SwedishAnalyzer(stopwords, CharArraySet::EMPTY_SET)
{
}
...
SwedishAnalyzer::createComponents(const wstring &fieldName)
{
  ...
  result = make_shared<SnowballFilter>(result, make_shared<SwedishStemmer>());
  return make_shared<Analyzer::TokenStreamComponents>(source, result);
}

shared_ptr<TokenStream> SwedishAnalyzer::normalize(const wstring &fieldName,

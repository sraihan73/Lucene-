 ...
 */
class TurkishAnalyzer final : public StopwordAnalyzerBase
{
  GET_CLASS_NAME(TurkishAnalyzer)
private:
...
public:
   ...
   */
  TurkishAnalyzer(std::shared_ptr<CharArraySet> stopwords);
...
protected:
  std::shared_ptr<TurkishAnalyzer> shared_from_this()
  {
    return std::static_pointer_cast<TurkishAnalyzer>(

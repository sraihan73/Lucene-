...
#include "../../../../tartarus/snowball/ext/ArmenianStemmer.h"
...
const wstring ArmenianAnalyzer::DEFAULT_STOPWORD_FILE = L"stopwords.txt";

shared_ptr<CharArraySet> ArmenianAnalyzer::getDefaultStopSet()
{
...
ArmenianAnalyzer::DefaultSetHolder::StaticConstructor::StaticConstructor()
{
  try {
    DEFAULT_STOP_SET = StopwordAnalyzerBase::loadStopwordSet(
        false, ArmenianAnalyzer::typeid, DEFAULT_STOPWORD_FILE, L"#");
...
DefaultSetHolder::StaticConstructor
    ArmenianAnalyzer::DefaultSetHolder::staticConstructor;
...
ArmenianAnalyzer::ArmenianAnalyzer(shared_ptr<CharArraySet> stopwords)
    : ArmenianAnalyzer(stopwords, CharArraySet::EMPTY_SET)
{
}
...
ArmenianAnalyzer::createComponents(const wstring &fieldName)
{
  ...
  result = make_shared<SnowballFilter>(result, make_shared<ArmenianStemmer>());
  return make_shared<Analyzer::TokenStreamComponents>(source, result);
}

shared_ptr<TokenStream> ArmenianAnalyzer::normalize(const wstring &fieldName,

...
/**
 * Factory for {@link SimplePatternSplitTokenizer}, for producing tokens by
 * splitting according to the provided regexp.
 *
 ...
 * </ul>
 * <p>
 * The pattern matches the characters that should split tokens, like {@code
 * std::wstring.split}, and the matching is greedy such that the longest token
 * separator matching at a given point is matched.  Empty tokens are never
 * created.
 *
 ...
 */
class SimplePatternSplitTokenizerFactory : public TokenizerFactory
{
  GET_CLASS_NAME(SimplePatternSplitTokenizerFactory)
public:
...
public:
  ...
  std::shared_ptr<SimplePatternSplitTokenizer>
  create(std::shared_ptr<AttributeFactory> factory) override;

protected:
  std::shared_ptr<SimplePatternSplitTokenizerFactory> shared_from_this()
  {
    return std::static_pointer_cast<SimplePatternSplitTokenizerFactory>(

...
#include  "core/src/java/org/apache/lucene/analysis/miscellaneous/TrimFilter.h"
 ...
 * &lt;fieldType name="text_trm" class="solr.TextField"
 * positionIncrementGap="100"&gt; &lt;analyzer&gt; &lt;tokenizer
 * class="solr.NGramTokenizerFactory"/&gt; &lt;filter
 * class="solr.TrimFilterFactory" /&gt; &lt;/analyzer&gt;
 * &lt;/fieldType&gt;</pre>
 *
 * @see TrimFilter
 */
class TrimFilterFactory : public TokenFilterFactory,
                          public MultiTermAwareComponent
{
  GET_CLASS_NAME(TrimFilterFactory)

  /** Creates a new TrimFilterFactory */
public:
  TrimFilterFactory(std::unordered_map<std::wstring, std::wstring> &args);

  std::shared_ptr<TrimFilter>
  create(std::shared_ptr<TokenStream> input) override;

  std::shared_ptr<AbstractAnalysisFactory> getMultiTermComponent() override;

protected:
  std::shared_ptr<TrimFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<TrimFilterFactory>(

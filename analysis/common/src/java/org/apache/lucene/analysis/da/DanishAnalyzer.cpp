...
#include "../../../../tartarus/snowball/ext/DanishStemmer.h"
...
const wstring DanishAnalyzer::DEFAULT_STOPWORD_FILE = L"danish_stop.txt";

shared_ptr<CharArraySet> DanishAnalyzer::getDefaultStopSet()
{
...
DanishAnalyzer::DefaultSetHolder::StaticConstructor::StaticConstructor()
{
  try {
    DEFAULT_STOP_SET =
...
DefaultSetHolder::StaticConstructor
    DanishAnalyzer::DefaultSetHolder::staticConstructor;
...
DanishAnalyzer::DanishAnalyzer(shared_ptr<CharArraySet> stopwords)
    : DanishAnalyzer(stopwords, CharArraySet::EMPTY_SET)
{
}
...
DanishAnalyzer::createComponents(const wstring &fieldName)
{
  ...
  result = make_shared<SnowballFilter>(result, make_shared<DanishStemmer>());
  return make_shared<Analyzer::TokenStreamComponents>(source, result);
}

shared_ptr<TokenStream> DanishAnalyzer::normalize(const wstring &fieldName,

...
#include  "core/src/java/org/apache/lucene/analysis/core/DecimalDigitFilter.h"
 ...
 * class="solr.DecimalDigitFilterFactory"/&gt; &lt;/analyzer&gt;
 * &lt;/fieldType&gt;</pre>
 * @since 5.4.0
 */
class DecimalDigitFilterFactory : public TokenFilterFactory,
                                  public MultiTermAwareComponent
{
  GET_CLASS_NAME(DecimalDigitFilterFactory)
...
public:
  ...
  std::shared_ptr<DecimalDigitFilter>
  create(std::shared_ptr<TokenStream> input) override;

  std::shared_ptr<AbstractAnalysisFactory> getMultiTermComponent() override;

protected:
  std::shared_ptr<DecimalDigitFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<DecimalDigitFilterFactory>(

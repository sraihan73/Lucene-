 ...
 */
class DanishAnalyzer final : public StopwordAnalyzerBase
{
  GET_CLASS_NAME(DanishAnalyzer)
private:
...
public:
   ...
   */
  DanishAnalyzer(std::shared_ptr<CharArraySet> stopwords);
...
protected:
  std::shared_ptr<DanishAnalyzer> shared_from_this()
  {
    return std::static_pointer_cast<DanishAnalyzer>(
        org.apache.lucene.analysis.StopwordAnalyzerBase::shared_from_this());
  }
};

} // #include  "core/src/java/org/apache/lucene/analysis/da/

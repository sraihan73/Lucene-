 ...
 * class="solr.KStemFilterFactory"/&gt; &lt;/analyzer&gt;
 * &lt;/fieldType&gt;</pre>
 * @since 3.3.0
 */
class KStemFilterFactory : public TokenFilterFactory
{
  GET_CLASS_NAME(KStemFilterFactory)

  /** Creates a new KStemFilterFactory */
public:
  KStemFilterFactory(std::unordered_map<std::wstring, std::wstring> &args);

  std::shared_ptr<TokenFilter>
  create(std::shared_ptr<TokenStream> input) override;

protected:
  std::shared_ptr<KStemFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<KStemFilterFactory>(

 ...
 */
class ItalianAnalyzer final : public StopwordAnalyzerBase
{
  GET_CLASS_NAME(ItalianAnalyzer)
private:
...
public:
   ...
   */
  ItalianAnalyzer(std::shared_ptr<CharArraySet> stopwords);
   ...
   *         {@link StandardFilter}, {@link ElisionFilter}, {@link
   * LowerCaseFilter}, {@link StopFilter} , {@link SetKeywordMarkerFilter} if a
   * stem exclusion set is provided and {@link ItalianLightStemFilter}.
   */
protected:
...
protected:
  std::shared_ptr<ItalianAnalyzer> shared_from_this()
  {
    return std::static_pointer_cast<ItalianAnalyzer>(

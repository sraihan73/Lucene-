...
#include "../../../../tartarus/snowball/ext/RussianStemmer.h"
...
const wstring RussianAnalyzer::DEFAULT_STOPWORD_FILE = L"russian_stop.txt";
const shared_ptr<org::apache::lucene::analysis::CharArraySet>
    RussianAnalyzer::DefaultSetHolder::DEFAULT_STOP_SET;

RussianAnalyzer::DefaultSetHolder::StaticConstructor::StaticConstructor()
{
  try {
    DEFAULT_STOP_SET =
...
DefaultSetHolder::StaticConstructor
    RussianAnalyzer::DefaultSetHolder::staticConstructor;

shared_ptr<CharArraySet> RussianAnalyzer::getDefaultStopSet()
{
...
RussianAnalyzer::RussianAnalyzer(shared_ptr<CharArraySet> stopwords)
    : RussianAnalyzer(stopwords, CharArraySet::EMPTY_SET)
{
}
...
RussianAnalyzer::createComponents(const wstring &fieldName)
{
  ...
  result = make_shared<SnowballFilter>(
      result, make_shared<org::tartarus::snowball::ext::RussianStemmer>());
  return make_shared<Analyzer::TokenStreamComponents>(source, result);
}

shared_ptr<TokenStream> RussianAnalyzer::normalize(const wstring &fieldName,

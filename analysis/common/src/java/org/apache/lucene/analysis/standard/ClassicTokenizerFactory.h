 ...
 * class="solr.ClassicTokenizerFactory" maxTokenLength="120"/&gt;
 *   &lt;/analyzer&gt;
 * &lt;/fieldType&gt;</pre>
 */
class ClassicTokenizerFactory : public TokenizerFactory
{
  GET_CLASS_NAME(ClassicTokenizerFactory)
private:
  const int maxTokenLength;

  /** Creates a new ClassicTokenizerFactory */
public:
  ClassicTokenizerFactory(std::unordered_map<std::wstring, std::wstring> &args);

  std::shared_ptr<ClassicTokenizer>
  create(std::shared_ptr<AttributeFactory> factory) override;

protected:
  std::shared_ptr<ClassicTokenizerFactory> shared_from_this()
  {
    return std::static_pointer_cast<ClassicTokenizerFactory>(

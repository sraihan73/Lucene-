...
#include  "core/src/java/org/apache/lucene/analysis/miscellaneous/ScandinavianFoldingFilter.h"
 ...
 * &lt;fieldType name="text_scandfold" class="solr.TextField"
 * positionIncrementGap="100"&gt; &lt;analyzer&gt; &lt;tokenizer
 * class="solr.WhitespaceTokenizerFactory"/&gt; &lt;filter
 * class="solr.ScandinavianFoldingFilterFactory"/&gt; &lt;/analyzer&gt;
 * &lt;/fieldType&gt;</pre>
 * @since 4.4.0
 */
class ScandinavianFoldingFilterFactory : public TokenFilterFactory,
                                         public MultiTermAwareComponent
{
  GET_CLASS_NAME(ScandinavianFoldingFilterFactory)

public:
  ScandinavianFoldingFilterFactory(
      std::unordered_map<std::wstring, std::wstring> &args);

  std::shared_ptr<ScandinavianFoldingFilter>
  create(std::shared_ptr<TokenStream> input) override;

  std::shared_ptr<AbstractAnalysisFactory> getMultiTermComponent() override;

protected:
  std::shared_ptr<ScandinavianFoldingFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<ScandinavianFoldingFilterFactory>(

...
{
  GET_CLASS_NAME(PayloadEncoder)

public:
  virtual std::shared_ptr<BytesRef> encode(std::deque<wchar_t> &buffer) = 0;

  /**
   * Convert a char array to a {@link BytesRef}
   * @return encoded {@link BytesRef}
   */
  virtual std::shared_ptr<BytesRef> encode(std::deque<wchar_t> &buffer,

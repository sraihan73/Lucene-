...
#include "../../../../tartarus/snowball/ext/BasqueStemmer.h"
...
const wstring BasqueAnalyzer::DEFAULT_STOPWORD_FILE = L"stopwords.txt";

shared_ptr<CharArraySet> BasqueAnalyzer::getDefaultStopSet()
{
...
BasqueAnalyzer::DefaultSetHolder::StaticConstructor::StaticConstructor()
{
  try {
    DEFAULT_STOP_SET = StopwordAnalyzerBase::loadStopwordSet(
        false, BasqueAnalyzer::typeid, DEFAULT_STOPWORD_FILE, L"#");
...
DefaultSetHolder::StaticConstructor
    BasqueAnalyzer::DefaultSetHolder::staticConstructor;
...
BasqueAnalyzer::BasqueAnalyzer(shared_ptr<CharArraySet> stopwords)
    : BasqueAnalyzer(stopwords, CharArraySet::EMPTY_SET)
{
}
...
BasqueAnalyzer::createComponents(const wstring &fieldName)
{
  ...
  result = make_shared<SnowballFilter>(result, make_shared<BasqueStemmer>());
  return make_shared<Analyzer::TokenStreamComponents>(source, result);
}

shared_ptr<TokenStream> BasqueAnalyzer::normalize(const wstring &fieldName,

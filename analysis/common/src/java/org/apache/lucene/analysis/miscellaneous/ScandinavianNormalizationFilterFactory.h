...
#include  "core/src/java/org/apache/lucene/analysis/miscellaneous/ScandinavianNormalizationFilter.h"
 ...
 * &lt;fieldType name="text_scandnorm" class="solr.TextField"
 * positionIncrementGap="100"&gt; &lt;analyzer&gt; &lt;tokenizer
 * class="solr.WhitespaceTokenizerFactory"/&gt; &lt;filter
 * class="solr.ScandinavianNormalizationFilterFactory"/&gt; &lt;/analyzer&gt;
 * &lt;/fieldType&gt;</pre>
 * @since 4.4.0
 */
class ScandinavianNormalizationFilterFactory : public TokenFilterFactory,
                                               public MultiTermAwareComponent
{
  GET_CLASS_NAME(ScandinavianNormalizationFilterFactory)

public:
  ScandinavianNormalizationFilterFactory(
      std::unordered_map<std::wstring, std::wstring> &args);

  std::shared_ptr<ScandinavianNormalizationFilter>
  create(std::shared_ptr<TokenStream> input) override;

  std::shared_ptr<AbstractAnalysisFactory> getMultiTermComponent() override;

protected:
  std::shared_ptr<ScandinavianNormalizationFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<ScandinavianNormalizationFilterFactory>(

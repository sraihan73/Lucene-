...
#include "../../../../tartarus/snowball/ext/FinnishStemmer.h"
...
const wstring FinnishAnalyzer::DEFAULT_STOPWORD_FILE = L"finnish_stop.txt";

shared_ptr<CharArraySet> FinnishAnalyzer::getDefaultStopSet()
{
...
FinnishAnalyzer::DefaultSetHolder::StaticConstructor::StaticConstructor()
{
  try {
    DEFAULT_STOP_SET =
...
DefaultSetHolder::StaticConstructor
    FinnishAnalyzer::DefaultSetHolder::staticConstructor;
...
FinnishAnalyzer::FinnishAnalyzer(shared_ptr<CharArraySet> stopwords)
    : FinnishAnalyzer(stopwords, CharArraySet::EMPTY_SET)
{
}
...
FinnishAnalyzer::createComponents(const wstring &fieldName)
{
  ...
  result = make_shared<SnowballFilter>(result, make_shared<FinnishStemmer>());
  return make_shared<Analyzer::TokenStreamComponents>(source, result);
}

shared_ptr<TokenStream> FinnishAnalyzer::normalize(const wstring &fieldName,

 ...
 * will not be indexed at all) and an external deque of exclusions (words that
will
...
GET_CLASS_NAME(uses)
 */
class BrazilianAnalyzer final : public StopwordAnalyzerBase
{
  GET_CLASS_NAME(BrazilianAnalyzer)
...
public:
   ...
   *          a stopword set
   */
  BrazilianAnalyzer(std::shared_ptr<CharArraySet> stopwords);
...
protected:
  std::shared_ptr<BrazilianAnalyzer> shared_from_this()
  {
    return std::static_pointer_cast<BrazilianAnalyzer>(

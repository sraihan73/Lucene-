...
const wstring BulgarianAnalyzer::DEFAULT_STOPWORD_FILE = L"stopwords.txt";

shared_ptr<CharArraySet> BulgarianAnalyzer::getDefaultStopSet()
{
...
BulgarianAnalyzer::DefaultSetHolder::StaticConstructor::StaticConstructor()
{
  try {
    DEFAULT_STOP_SET = StopwordAnalyzerBase::loadStopwordSet(
        false, BulgarianAnalyzer::typeid, DEFAULT_STOPWORD_FILE, L"#");
...
DefaultSetHolder::StaticConstructor
    BulgarianAnalyzer::DefaultSetHolder::staticConstructor;
...
BulgarianAnalyzer::BulgarianAnalyzer(shared_ptr<CharArraySet> stopwords)
    : BulgarianAnalyzer(stopwords, CharArraySet::EMPTY_SET)
{
}
...
BulgarianAnalyzer::createComponents(const wstring &fieldName)
{
  ...
  result = make_shared<BulgarianStemFilter>(result);
  return make_shared<Analyzer::TokenStreamComponents>(source, result);
}

shared_ptr<TokenStream>
BulgarianAnalyzer::normalize(const wstring &fieldName,

 ...
 *  <p>
 *  <code>Collation</code> converts each token into its binary
 * <code>CollationKey</code> using the provided <code>Collator</code>, allowing
 ...
 *     use non-Unicode character orderings.  (Range queries using a Locale can
 * be very slow.)
 *   </li>
 *   <li>
 *     Effective Locale-specific normalization (case differences, diacritics,
 * etc.).
 *     ({@link org.apache.lucene.analysis.LowerCaseFilter} and
 *     {@link org.apache.lucene.analysis.miscellaneous.ASCIIFoldingFilter}
 * provide these services in a generic way that doesn't take into account
 ...
 * IndexWriterConfig(analyzer)); Document doc = new Document(); doc.add(new
 * TextField("content", "\u0633\u0627\u0628", Field.Store.YES));
 ...
 * IndexWriterConfig(analyzer)); std::wstring[] tracer = new std::wstring[] { "A", "B", "C",
 * "D", "E" }; std::wstring[] data = new std::wstring[] { "HAT", "HUT", "H\u00C5T",
 * "H\u00D8T", "HOT" }; std::wstring[] sortedTracerOrder = new std::wstring[] { "A", "E",
 * "B", "D", "C" }; for (int i = 0 ; i &lt; data.length ; ++i) { Document doc =
 * new Document(); doc.add(new StoredField("tracer", tracer[i])); doc.add(new
 * TextField("contents", data[i], Field.Store.NO)); writer.addDocument(doc);
 *   }
 *   writer.close();
 ...
 *   IndexWriter writer = new IndexWriter(ramDir, new
 * IndexWriterConfig(analyzer)); Document doc = new Document(); doc.add(new
 * TextField("contents", "DIGY", Field.Store.NO)); writer.addDocument(doc);
 *   writer.close();
 *   IndexReader ir = DirectoryReader.open(ramDir);
 *   IndexSearcher is = new IndexSearcher(ir);
 ...
 * </ol>
 * <p>
 *   <code>ICUCollationKeyAnalyzer</code>, available in the <a
 * href="{@docRoot}/../analyzers-icu/overview-summary.html">icu analysis
 * module</a>, uses ICU4J's <code>Collator</code>, which makes its version
 * available, thus allowing collation to be versioned independently from the
 * JVM.  <code>ICUCollationKeyAnalyzer</code> is also significantly faster and
 * generates significantly shorter keys than <code>CollationKeyAnalyzer</code>.
 ...
 * </p>
 * <p>
 *   <code>CollationKey</code>s generated by <code>java.text.Collator</code>s
 * are not compatible with those those generated by ICU Collators. Specifically,
 * if you use <code>CollationKeyAnalyzer</code> to generate index terms, do not
 * use <code>ICUCollationKeyAnalyzer</code> on the query side, or vice versa.
 * </p>
 */

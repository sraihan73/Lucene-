...
#include "TestCollationDocValuesField.h"
...
#include "../../../../../java/org/apache/lucene/collation/CollationDocValuesField.h"
...
void TestCollationDocValuesField::testBasic() 
{
  ...
  shared_ptr<CollationDocValuesField> collationField =
      make_shared<CollationDocValuesField>(
          L"collated", Collator::getInstance(Locale::ENGLISH));
  doc->push_back(field);
...
void TestCollationDocValuesField::testRanges() 
{
  shared_ptr<Directory> dir = newDirectory();
  shared_ptr<RandomIndexWriter> iw =
      make_shared<RandomIndexWriter>(random(), dir);
  ...
  shared_ptr<Collator> collator =
      Collator::getInstance(Locale::getDefault()); // uses -Dtests.locale
  if (random()->nextBoolean()) {
    collator->setStrength(Collator::PRIMARY);
  }
  shared_ptr<CollationDocValuesField> collationField =
      make_shared<CollationDocValuesField>(L"collated", collator);
  doc->push_back(field);
  doc->push_back(collationField);
...
void TestCollationDocValuesField::doTestRanges(
    ...
    shared_ptr<Collator> collator) 
{
  ...
  for (int docID = 0; docID < is->getIndexReader()->maxDoc(); docID++) {
    ...
    bool collatorAccepts = collate(collator, s, startPoint) >= 0 &&
                           collate(collator, s, endPoint) <= 0;
    TestUtil::assertEquals(docID, dvs->nextDoc());
    shared_ptr<BytesRef> br = dvs->binaryValue();
    bool luceneAccepts =
        br->compareTo(startBR) >= 0 && br->compareTo(endBR) <= 0;
    assertEquals(startPoint + L" <= " + s + L" <= " + endPoint, collatorAccepts,
                 luceneAccepts);
  }
}
} // namespace org::apache::lucene::collation
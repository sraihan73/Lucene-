 ...
 */
class TestNorwegianMinimalStemFilterFactory
    : public BaseTokenStreamFactoryTestCase
{
  GET_CLASS_NAME(TestNorwegianMinimalStemFilterFactory)
public:
  virtual void testStemming() ;

  /** Test stemming with variant set explicitly to Bokmål */
  virtual void testBokmaalStemming() ;

  /** Test stemming with variant set explicitly to Nynorsk */
  virtual void testNynorskStemming() ;

  /** Test that bogus arguments result in exception */
  virtual void testBogusArguments() ;

protected:
  std::shared_ptr<TestNorwegianMinimalStemFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<TestNorwegianMinimalStemFilterFactory>(

...
#include "TestAllDictionaries2.h"
...
const shared_ptr<java::nio::file::Path> TestAllDictionaries2::DICTIONARY_HOME =
    java::nio::file::Paths->get(L"/data/thunderbirdDicts");

void TestAllDictionaries2::test() 
{
  ...
  for (int i = 0; i < tests.size(); i += 3) {
    ...
    // java.nio.file.Files.newInputStream(f))
    {
      ...
      // org.apache.lucene.store.Directory tempDir = newDirectory())
      {
...
void TestAllDictionaries2::testOneDictionary() 
{
  shared_ptr<Path> tmp = LuceneTestCase::createTempDir();

  wstring toTest = L"hungarian_dictionary-1.6.1.1-fx+tb+sm+fn.xpi";

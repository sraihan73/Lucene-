...
#include  "core/src/java/org/apache/lucene/collation/ICUCollationAttributeFactory.h"
...
/**
 * <p>
 *   Configures {@link KeywordTokenizer} with {@link
 * ICUCollationAttributeFactory}. <p> Converts the token into its {@link
 * com.ibm.icu.text.CollationKey}, and then encodes the CollationKey directly to
 ...
 * </p>
 */
class ICUCollationKeyAnalyzer final : public Analyzer
{
  GET_CLASS_NAME(ICUCollationKeyAnalyzer)
private:
  const std::shared_ptr<ICUCollationAttributeFactory> factory;
   ...
   */
public:
  ICUCollationKeyAnalyzer(std::shared_ptr<Collator> collator);
...
protected:
  std::shared_ptr<ICUCollationKeyAnalyzer> shared_from_this()
  {
    return std::static_pointer_cast<ICUCollationKeyAnalyzer>(

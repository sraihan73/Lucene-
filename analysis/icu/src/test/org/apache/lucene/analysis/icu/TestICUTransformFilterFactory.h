...
class TestICUTransformFilterFactory : public BaseTokenStreamTestCase
{
  GET_CLASS_NAME(TestICUTransformFilterFactory)
...
public:
  ...
  virtual void testForwardDirection() ;

  virtual void testReverseDirection() ;
...
private:
  ...
  private:
    std::shared_ptr<TestICUTransformFilterFactory> outerInstance;
...
protected:
  std::shared_ptr<TestICUTransformFilterFactory> shared_from_this()
  {
    return std::static_pointer_cast<TestICUTransformFilterFactory>(
